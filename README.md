...
**Domain:** `Finance / Expense Ops` · **Suite:** `Finance Document Suite` · **Accent:** `#67e8f9`
...
**Expense Categorizer** is a standalone, browser-based AI workflow app for **Finance / Expense Ops**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
| `business_personal`<br>business/personal | text | No | — | Affects customization: business/personal. |
| `vat_tax_mode`<br>VAT/tax mode | select | No | — | Affects customization: VAT/tax mode. |
| `category_rules`<br>category rules | textarea | No | — | Affects customization: category rules. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9121
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9121`. |
...
```bash
curl http://127.0.0.1:9121/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9121/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9121 uvicorn app.main:app --host 0.0.0.0 --port 9121
```
...
- `plugins/expense-categorizer.json` for plugin metadata.
